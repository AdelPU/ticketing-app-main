...
class HISPortalApp {
  ...
  setupNavigationListeners() {
    ...
    document
      .querySelectorAll(".card-action-link[data-section]")
      .forEach((link) => {
        link.addEventListener("click", (e) => {
          e.preventDefault();
          this.navigateToSection(link.dataset.section);
  ...
  setupKeyboardShortcuts() {
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Escape") {
        ...
        if (
          (ticketModal && ticketModal.style.display === "flex") ||
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    app.showInitError(
      "Critical error during initialization. Please refresh the page."
    );
  }
});
