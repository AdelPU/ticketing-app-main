    ...
    try {
      ...
      if (selectedFile) {
        try {
          ...
          if (updateError) {
            console.error(
              "Error updating ticket with screenshot:",
              updateError
            );
            showToast(
      ...
      <p>Ticket ID: <strong>${ticket.ticket_number}</strong></p>
    `,
        });

        const eData = await window.getEByI();
        const emails = eData.emails;
        ...
        `,
          ...
          console.warn("No emails returned or invalid format");
        }
      } catch (err) {
        console.warn("Ticket submitted but email failed: ", err);
        showToast("Ticket submitted, but email could not be sent.", "warning");
      }
